...
| 函数 | 用途 | maxTokens |
|------|------|-----------|
| `callAI()` | 主游戏循环行动生成 | 10000（实际上限 8192）|
| `generateRestActionWithAI()` | 休息行动生成 | 500 |
| `callNightlyReviewAI()` | 夜间关系审查 | 1500 |
| `generateArtworkImagePrompt()` | 美术图像 prompt | 2400 |
...
- **现象：** 角色无法从房间消耗物品，静默失败
- **根因：** system prompt schema 定义字段为 `consumed.from`，代码检查的是 `consumed.room`
- **影响：** AI 按 schema 返回 `from: "kitchen"` → 代码读 `.room` 为 undefined → 房间消耗跳过
- **只有** `from: "inventory"` 路径正常工作
- **修复方向：** 将对应行的 `consumed.room` 改为 `consumed.from`

---

### Bug 2：energy 下限是 1 而非 0 ⚠️ 高优先级（已部分处理）

- **代码：** `Math.max(1, ...)`
- **现象：** 体力永远无法降到 0，"体力耗尽昏倒"机制无法触发
- **当前状态：** 晕倒判断已用 `if (false && ...)` 临时禁用，精力最低保持 1
- **修复方向：** 改为 `Math.max(0, ...)`，重新启用晕倒逻辑
...
- **影响：** 自定义角色数量不是 3 时，mock 产生无效动作
- **修复方向：** 改为动态读取 `gameState.characters`

---

### Bug 4：夜间关系审查不启用 JSON 格式 🟡 中优先级

- **描述：** `callNightlyReviewAI` 传 `useJsonFormat: false`，prompt 要求 JSON 但不强制结构化输出
- **修复方向：** 改为 `useJsonFormat: true`
...
- **多阶段 JSON 修复管线**（`cleanAndParseJSON` + `repairJSONStrings`）：字符级状态机 + 6 级降级解析（直接解析 → repairJSONStrings → 位置截断 → 末尾逐候选截断 → Unterminated 激进修复 → 基础提取），鲁棒性很高
- **gameLoop 异常保护**：try/catch/finally 确保任何崩溃都不会卡死循环，`isProcessing` 始终重置
- **对话质量过滤**：独白检测（speakers.size < 2 跳过）+ 互动标题去重（shownInteractionPairs Set）
- **动态状态注释**（`generateDynamicPersonality`）：实时将数值状态转换为语义描述，有效引导 AI 输出
